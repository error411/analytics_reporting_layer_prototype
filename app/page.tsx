...
function HorizontalReportChart({ points, groupBy }: { points: AggregatedPoint[]; groupBy: string }) {
  // Memoize bucket preparation so unrelated dashboard state does not rebuild the chart.
    ...
    <div className="report-chart">
      ...
      <div className="bar-grid">
        {chartPoints.map((point) => {
          const viewsPercent = (point.views / maxViews) * 100;
            ...
            <div className="bar-row" key={point.label}>
              ...
              <div className="bar-track">
                <div className="bar-line">
                  ...
                  />
                  <b style={barLabelStyle(point.averageEngagementScore)}>
                    {point.averageEngagementScore}
                  </b>
                </div>
                <div className="bar-line">
                  <span className="bar bar--views" style={{ width: `${viewsPercent}%` }} />
                  <b style={barLabelStyle(viewsPercent)}>{formatNumber(point.views)}</b>
                </div>
              </div>
            </div>
          );
    ...
    <table>
      ...
      <tbody>
        {report.topArticles.map((article) => (
          <tr key={`${article.category}:${article.title}`}>
            <td>{article.title}</td>
...
function prepareChartPoints(points: AggregatedPoint[], groupBy: string): AggregatedPoint[] {
  ...
  // Date-grouped reports are condensed to months to match the reference chart density.
...
function barLabelStyle(percent: number) {
  if (percent > 84) {
    return {
      right: `${100 - percent}%`,
